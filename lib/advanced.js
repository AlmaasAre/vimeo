...
var get = function(url, auth, cb) {
  oauth.get(url, jakobKeys.token, jakobKeys.tokenSec, function(err, res) {
...
var getRequestToken = function(redirect, perms, cb) {
  ...
  oauth.getOAuthRequestToken(params, function(err, token, secret) {
    if (err !== null) return cb(err.data, null)
...
module.exports = function(key, sec, token, tokenSec) {
  if(typeof oauth !== 'object')
    oauth = new oauth(urls.request, urls.access, key, sec, '1.0', null, 'HMAC-SHA1')

  jakobKeys.token = token;
  jakobKeys.tokenSec = tokenSec;
